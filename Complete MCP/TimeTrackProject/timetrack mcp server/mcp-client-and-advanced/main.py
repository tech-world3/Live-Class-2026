...
# ============================================================
# NEW: a tool that uses SAMPLING -- the server borrows the
# CONNECTED CLIENT's own LLM to write the summary, instead of
# formatting the numbers itself.
# ============================================================

@mcp.tool
async def summarize_week(employee_name: str, week_start: str, ctx: Context) -> str:
    """Summarize one employee's week in plain language, using the client's own LLM."""
    ...
    # Step 3: this is the actual sampling call. It PAUSES this tool and
    # sends a request out to whatever client is connected, asking IT to
    # generate the text. This tool never talks to any LLM provider itself.
    result = await ctx.sample(
        f"Summarize this week's work for {employee_name} in two friendly sentences:\n\n{entries_text}"
    )
    return result.text or "Could not generate a summary."
